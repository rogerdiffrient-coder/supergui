...
const [runtime, gameServices, model, editor, categories, groupedCategories, bundle] = await Promise.all([
  ...
  read('src/grouped-categories.js'),
...
assert.ok(bundle.includes('registerSuperGUICategories(core)'), 'bundle does not register grouped categories');
assert.ok(groupedCategories.includes('new Proxy') && groupedCategories.includes('value.bind(core)'), 'grouped category wrappers do not delegate to the shared core');
assert.ok(categories.includes('sg612InstallHatRouting'), 'grouped category hats are not routed back to the shared core');
...
console.log(`OK: ${opcodes.length} core blocks, ${methods.size} methods, categorized editor, grouped shared-core toolbox, current bundle`);
