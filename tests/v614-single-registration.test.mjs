...
assert.equal((bundle.match(/Scratch\.extensions\.register\(core\)/g) || []).length, 1, 'bundle must register the SuperGUI core exactly once');
assert.ok(!/Scratch\.extensions\.register\((?:proxy|new\s+SuperGUI|new\s+\w+Category)/.test(bundle), 'bundle must not register proxy/subextensions');
...
assert.ok(palette.includes('SG607_CATEGORIES'), 'palette navigation must remain available');
